...
--prompt V 3.8 - Modified on April 24, 2023, this version will be oriented to 12c and later versions, to retrieve information regarding Multitenant containers.
--prompt V 3.7 - Modified on November 17, 2022, creating conditions and variables to allow different outputs if the DB version is pre-19 or 19c 
...
select '<body><div align=left><b><font face="Arial,Helvetica"><font color="#ff0000">' ||
       '<font size=-2>' || to_char(sysdate, 'DD-MON-YYYY HH24:MI:SS') || ' Ver 3.8 ' ||
...
--START Determine if the DB is pre19 or 19c 
define DB_VER = 'No Version'
column DB_VER_NUM new_val DB_VER NOPRINT
select  CASE WHEN TO_NUMBER(SUBSTR(banner,17,2)) < 18 THEN 'banner' ELSE 'banner_full' END DB_VER_NUM from v$version where rownum = 1;
--END Determine if the DB is pre19 or 19c 

--START Determine if the DB is standard or Multitenant 
define DB_CONT = 'Standard DB'
column DB_CONT_NUM new_val DB_CONT NOPRINT
select CASE TO_NUMBER(SYS_CONTEXT('USERENV', 'CON_ID'))  WHEN 0 THEN 'CDB/Standard' WHEN 1 THEN 'CDB' WHEN 2 THEN 'PDB$SEED' ELSE 'Custom PDB' END DB_CONT_NUM from dual;
--END Determine if the DB is standard or Multitenant
...
select '<TR><TD BGCOLOR=#FFFFFF><FONT FACE="ARIAL" SIZE=2> ' || &DB_VER ||
...
select '<h5><font face="VERDANA"><font color="#000000">APEX Multitenant Information' ||
       '<font size=-2></font></font></font></h5>' FROM dual;
select '<TABLE BORDER  CELLPADDING=2>' FROM dual;
select '<TH BGCOLOR=#33ccff><B><FONT FACE="ARIAL" COLOR="#FFFFFF" SIZE=2>' || 'Name </FONT></B></TH>' FROM dual;
select '<TH BGCOLOR=#33ccff><B><FONT FACE="ARIAL" COLOR="#FFFFFF" SIZE=2>' || 'DB Type </FONT></B></TH>' FROM dual;
select '<TH BGCOLOR=#33ccff><B><FONT FACE="ARIAL" COLOR="#FFFFFF" SIZE=2>' || 'Open mode </FONT></B></TH>' FROM dual;
select '<TH BGCOLOR=#33ccff><B><FONT FACE="ARIAL" COLOR="#FFFFFF" SIZE=2>' || 'Con ID </FONT></B></TH>' FROM dual;
select '<TR><TD BGCOLOR=#FFFFFF><FONT FACE="ARIAL" SIZE=2> ' || NAME || '</FONT></TD>',
       '<TD BGCOLOR=#FFFFFF><FONT FACE="ARIAL" SIZE=2> ' || NVL(DECODE(CDB, 'YES', 'Multitenant', 'Standard'),'Standard') || '</FONT></TD>',
	   '<TD BGCOLOR=#FFFFFF><FONT FACE="ARIAL" SIZE=2> ' || OPEN_MODE || '</FONT></TD>',
	   '<TD BGCOLOR=#FFFFFF><FONT FACE="ARIAL" SIZE=2> ' || CON_ID || 
       '</FONT></TD></TR>'from v$database;
select '</TABLE>' FROM dual;
...
select '<h5><font face="VERDANA"><font color="#000000">Which container am I connected to?' ||
       '<font size=-2></font></font></font></h5>' FROM dual;
select '<TABLE BORDER  CELLPADDING=2>' FROM dual;
select '<TH BGCOLOR=#33ccff><B><FONT FACE="ARIAL" COLOR="#FFFFFF" SIZE=2>' || 'PDB/CDB/PDB$SEED </FONT></B></TH>' FROM dual;
select '<TR><TD BGCOLOR=#FFFFFF><FONT FACE="ARIAL" SIZE=2> ' || ''' &DB_CONT ''' ||  ' </FONT></TD></TR>' FROM dual;
select '</TABLE>' FROM dual;
...
exec dbms_output.put_line( '<TH BGCOLOR=#33ccff><B><FONT FACE="ARIAL" COLOR="#FFFFFF" SIZE=2>' || 'Is Common </B></FONT></TH>');
select '<TR><TD BGCOLOR=#FFFFFF><FONT FACE="ARIAL" SIZE=2> ' || username ||'</FONT></TD>',
           ...
           '<TD BGCOLOR=#FFFFFF><FONT FACE="ARIAL" SIZE=2> ' || common || '</FONT></TD></TR>'
from dba_users
where (username like 'APEX%' or username = 'FLOWS_FILES') and
username not in (select  username from dba_users where (username like 'APEX\_0%' escape '\' or
                  username like 'FLOWS\_0%' escape '\' 
                  or username like 'APEX\_2%' escape '\' or 
                  username like 'APEX\_1%' escape '\') and (username <> '&APEX'))
...
select '<TR><TD BGCOLOR=#FFFFFF><FONT FACE="ARIAL" SIZE=2> ' || username ||'</FONT></TD></TR>' 
       from dba_users where (username like 'APEX\_0%' escape '\' or username like 'FLOWS\_0%' escape '\'  or username like 'APEX\_2%' escape '\' or 
                  username like 'APEX\_1%' escape '\') and (username <> '&APEX') order by username asc;
exec dbms_output.put_line('</TABLE>');
